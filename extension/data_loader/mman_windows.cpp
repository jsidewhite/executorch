...
HRESULT try_grow_process_memory_working_set(DWORD dwSizeRequired) {
  ...
  if (!GetProcessWorkingSetSize(GetCurrentProcess(), &minWorkingSetInitial,
                                &maxWorkingSet)) {
    return GetLastError();
  }
  ...
  // Grow working set
  if (!SetProcessWorkingSetSize(GetCurrentProcess(), minWorkingSet,
                                maxWorkingSet)) {
    return GetLastError();
  }
  return S_OK;
...
HRESULT virtual_lock_allowing_working_set_growth(void* pMem,
                                                         DWORD dwSize) {
...
int mlock(const void *addr, size_t len) {
  HRESULT hr =
      virtual_lock_allowing_working_set_growth((LPVOID)addr, len);
  if (SUCCEEDED(hr)) {
    return 0;
  }
